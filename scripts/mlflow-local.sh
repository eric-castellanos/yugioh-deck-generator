...
setup_aws_credentials() {
    ...
    if command -v aws &> /dev/null; then
        ...
        # Try to get temporary credentials using the setup-mlflow role
        if aws sts get-caller-identity &> /dev/null; then
            ...
            if [ ! -z "$TEMP_CREDS" ]; then
                ...
                echo "⚠️  Could not assume role, using current credentials"
            fi
        else
...
case "${1:-menu}" in
    ...
    "menu"|*)
        ...
        echo "============================="
        echo "1. ./scripts/mlflow-local.sh start    # Start MLflow"
        echo "2. Open: http://localhost:5000"
        echo ""
        echo "AWS Mode (current setup):"
        echo "- Direct connection to public RDS database"
        echo "- Uses your existing S3 bucket"
        echo "- Available at: http://localhost:5000"
        echo "- Cost optimized: ~\$18/month (was \$103/month)"
        echo ""
        echo "Offline Mode (for complete offline work):"
