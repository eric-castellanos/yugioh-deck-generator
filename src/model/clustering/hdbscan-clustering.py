"""
HDBSCAN clustering experiment for Yu-Gi-Oh card data with comprehensive MLflow tracking.
"""
...
def run_clustering_experiment(feature_type: Literal["tfidf", "embeddings", "combined"]):
    """Run HDBSCAN clustering experiment with specified feature type."""
    ...
    with mlflow.start_run():
        ...
        # Apply PCA for dimensionality reduction before HDBSCAN
        ...
        # HDBSCAN works better with standardized features
        logging.info("Standardizing features for HDBSCAN")
        scaler = StandardScaler()
        X_scaled = scaler.fit_transform(X_reduced)

        logging.info(f"Running HDBSCAN with min_cluster_size={MIN_CLUSTER_SIZE}, min_samples={MIN_SAMPLES}")
        ...
        if hasattr(clusterer, 'condensed_tree_'):
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp_file:
                ...
                plt.title(f"HDBSCAN Cluster Hierarchy - {feature_type.upper()} Features")
                plt.tight_layout()
                plt.savefig(tmp_file.name, dpi=300, bbox_inches='tight')
                plt.close()
        ...
        with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp_file:
            ...
            plt.title(f"t-SNE Projection of HDBSCAN Clusters - {feature_type.upper()} Features")
            plt.xlabel("t-SNE 1")
            plt.ylabel("t-SNE 2")
...
def main():
    parser = argparse.ArgumentParser(description="Run HDBSCAN clustering with different feature types")
    parser.add_argument(
        "--feature-type", 
        type=str, 
    ...
    if args.feature_type == "all":
        ...
        for feature_type in ["tfidf", "embeddings", "combined"]:
            logging.info(f"🚀 Starting {feature_type} HDBSCAN clustering experiment...")
            run_clustering_experiment(feature_type)
            logging.info(f"✅ Completed {feature_type} HDBSCAN clustering experiment")
    else:
        # Run single experiment
        logging.info(f"🚀 Starting {args.feature_type} HDBSCAN clustering experiment...")
        run_clustering_experiment(args.feature_type)
        logging.info(f"✅ Completed {args.feature_type} HDBSCAN clustering experiment")
